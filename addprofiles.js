...
var ff = new gf.GridFloat( "/storage/maps/data/58735568/58735568" )
console.log( ff );
...
server.open(function(err, client) {
  ...
  var collection = new mongodb.Collection(client,"profiles");
      ...
      cursor.nextObject( function(err,doc){
        if(doc){
          ...
          for( var way in doc.value.ways ){
            ...
            //  debugger;
            //}
            var profile = {'id':doc.value.ways[way].id, 'profile':ff.profile( loc )};
            if( profile===null ){
              continue;
            }
            profiles.push( profile );
          }
          collection.insert(profiles, {safe:true}, function(err,obj){
            console.log( doc._id+" profiles inserted" );
          })
